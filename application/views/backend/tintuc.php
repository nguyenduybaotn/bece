...
<div class="row m-0">
    ...
    <p>
        ...
        <a href='<?php echo base_url("ckfinder/created_ses_ck.php?ses=login_ck&redirect=".base_url('backend/themtintuc')); ?>' class="btn btn-success btn-sm" >
    ...
    <table class="table table-bordered">
        ...
        <tbody class='table_danhsachtintuc'>
            ...
            $numberperpage = 10;
            $page=0;
            $total = count($this->tool_model->get_all_table_where('tintuc',"loai=0"));
            if(!isset($_GET['page'])){
                $page = 0;
                $limit_s = 0;
            ...
            $i=1;
            foreach($this->tool_model->get_all_table_where('tintuc',"loai=0 order by sapxep desc limit $limit_s,$limit_e") as $row){
                ...
                $url_ses = base_url("ckfinder/created_ses_ck.php?ses=login_ck&redirect=".base_url('backend/suatintuc/'));
                echo "<tr>
                    <td>".$stt."</td>
                    ...
                    <button type='button' class='btn btn-danger btn-sm'><a style='color:white;text-decoration:none;' href='".base_url('backend/xoatintuc/').$row->id."'><i class='fa fa-remove'></i>&nbsp;&nbsp;Xóa</a></button>
                    ";
                    echo "</td>
                </tr>";
                $i++;
    ...
    <p class='paginationx mt-5'>
        <?php  if($limit_s==0 && ($limit_e+$limit_s) <= $total) { ?>
                    <a href="<?php echo base_url()."backend/tintuc?page=".++$page; ?>">Next</a>
        <?php }elseif($limit_s>0 && ($limit_e+$limit_s) < $total ){ $pn = $page+1; $pp = $page-1; ?>
                    <a href="<?php echo base_url()."backend/tintuc?page=".$pp; ?>">Preview</a>
                    <a href="<?php echo base_url()."backend/tintuc?page=".$pn; ?>">Next</a>
        <?php }else if(($limit_e+$limit_s) >= $total && $total>=$numberperpage) { ?>
                    <a href="<?php echo base_url()."backend/tintuc?page=".--$page; ?>">Preview</a>
        <?php }?>
    </p>
    <style>
        .paginationx{
